const { v4: uuid } = require('uuid')//gerar codigo aleatorio
const jimp = require('jimp');// para manipulação de imagem
...
//gera um nome para imagem e configura
const addImage = async (buffer) => {
    let newName = `${uuid()}.jpg`;
    let tmpImg = await jimp.read(buffer);
    //diminuir tamanho de uma imagem sem perder qualidade
...
module.exports = {
    ...
    addAction: async (req, res) => {
        ...
        if(price){
            ...
            //recebe dados do tipo string e transforma em float ou seja numero
        ...
        //literalmente adcionando um anuncio
        const newAd = new Ad();
        newAd.status = true;
    ...
    editAction: async (req, res) => {
        ...
        //verificar se id do anuncio é valido
        if(id.length < 12) {
            res.json({error: 'ID inválido'});
            return;
        }
        ...
        //verificar se usuario é dono do anuncio
        const user = await User.findOne({token}).exec();
